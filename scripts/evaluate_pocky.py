...
from scripts.assemble_cub_birds import assemble_images
...
def parse_args(args):
    ...
    parser.add_argument("--group-size", type=int, default=1, help="num images to put in a single LLM query")
...
def get_ci(entropies):
    ...
    std_dev_ent = np.sqrt(np.var(entropies)/entropies.size)
    return np.array([
        mean_ent,
...
def load_images(args):
    if args.dataset_folder is None:
        ...
        attributes_file = os.path.join(args.dataset_folder, "attributes.txt")
        images_attribute_file = os.path.join(args.dataset_folder, "CUB_200_2011/attributes/image_attribute_labels_clean.txt")
        images_df = assemble_images(image_labels_file, images_file, attributes_file, image_folder, images_attribute_file)
        
        # select random bird class
        candidate_labels = images_df.orig_bird_label[~(images_df.orig_bird_label.isin(args.trained_classes))].unique()
        rand_class = np.random.choice(candidate_labels)
        target_domain = f"class {rand_class}"
        logging.info("selected random class %d", rand_class)
        
        test_data_df = images_df[images_df.orig_bird_label == rand_class].iloc[:args.max_obs]
    return test_data_df, target_domain
